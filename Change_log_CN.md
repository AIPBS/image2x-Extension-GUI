...
---
## v3.136.01
- 新模型： [AnimeVideo-Mini] 模型的 v1.8 更新，改进了去噪和细节重建。
- 改进了对高 DPI 显示器的支持。
- 修复了打开本地 HTML 帮助文档时浏览器出现模糊外观的问题。
- 解决了在某些电脑上本地帮助文档无法正常打开的问题。
- 修复 bug：放大极小图像时引擎崩溃。
---
## v3.135.01
- 通过进一步的流水线优化，实现更快的图像批处理。
- 优化代码以提升 [画面分析] 的性能。
- 新功能：[从不转换为 MP4]，完全跳过兼容性检查和 MP4 转换。（位置：附加设置 标签页）
- 改进 UI 并调整部分 UI 组件。
- 补全缺失的翻译。
---
## v3.134.01
...
- 更智能地处理极高分辨率图像，更好地适应不同的超分辨率引擎和设置。
- 加快了对 RTX 50 系列的兼容性测试。
- 更新了部分控制台消息。
---
## v3.132.01
- 改进了 [速度提升] (帧分析) 功能，以实现更快匹配。
- 改进了 [场景切换检测] (帧分析) 功能，以实现更快检测。
- 增强了对高分辨率图像的处理：
- Waifu2x 现在会在放大处理前自动缩小极高分辨率图像，以防止引擎故障。
- 此功能仅自动缩小会导致引擎崩溃的超高分辨率图像，将其调整到最大支持尺寸，以最大限度地减少细节损失。
- 通过修复可能导致内存泄漏和崩溃的潜在问题，提高了代码的健壮性。
---
## v3.131.01
...
- 该新模型的速度与 Omni-Mini-W2xEX 相似。
- 新功能：为自定义分辨率预设添加 [自动应用] 开关。启用后，在切换预设时，所选预设将自动应用于列表中的所有文件。
- 新功能：为自定义分辨率预设添加 [默认] 预设，以便更轻松地切换回默认的自定义分辨率设置。
- 新功能：在 [移除所选文件] 按钮的右键菜单中添加 [仅保留失败文件] 选项。
- 提升了与 RTX 50 系列 GPU 的兼容性。
---
## v3.127.01
