...
int MainWindow::RealESRGAN_NCNN_Vulkan_Image(int rowNum,bool ReProcess_MissingAlphaChannel)
{
    ...
    for(int i=Initial_ScaleRatio; i<=ScaleRatio_tmp; i*=Initial_ScaleRatio)
    {
        ...
        const QString cmd = "\"" + program + "\" -i \"" + inputPath
            + "\" -o \"" + outputPath + "\" -s "
            + QString::number(Initial_ScaleRatio, 10) + " "
    ...
    const BackendProcessResult result = backendClient->runImageStagesBlocking(
        QStringLiteral("realesrgan-ncnn-vulkan"), modelRecord, stages, 120000,
        ui->spinBox_retry->value() + ForceRetryCount - 1);
    QString OutputPath_tmp = stages.isEmpty() ? QString() : stages.last().outputPath;
    if (!result.succeeded())
    {
        qWarning().noquote() << "[image2x] realesrgan-ncnn-vulkan failed:"
                             << result.diagnostic;
