    ...
    for(int retry=0; retry<(ui->spinBox_retry->value()+ForceRetryCount); retry++)
    {
        ...
        for(int i=Initial_ScaleRatio; i<=ScaleRatio_tmp; i*=Initial_ScaleRatio)
        {
            ...
            if (!Waifu2x->waitForStarted(10000))
            {
                ...
                qWarning().noquote() << "[image2x] realesrgan-ncnn-vulkan failed to start:"
                                     << Waifu2x->errorString();
                break;
            }
            qWarning().noquote() << "[image2x] realesrgan-ncnn-vulkan started with pid"
                                 << Waifu2x->processId();
            while(!Waifu2x->waitForFinished(500)&&!QProcess_stop)
            {
                LogEngineProgress(QStringLiteral("realesrgan-ncnn-vulkan"), Waifu2x,
            ...
            qWarning().noquote() << "[image2x] realesrgan-ncnn-vulkan finished with exit code"
...
int MainWindow::RealESRGAN_NCNN_Vulkan_GIF(int rowNum)
{
    ...
    QMap<QString,int> result_map = Calculate_ScaleRatio_RealESRGAN_NCNNVulkan(ScaleRatio_Original);
    ...
    //读取配置讯息
    QString RealESRGAN_NCNN_Vulkan_Settings_str = RealESRGAN_NCNN_Vulkan_ReadSettings_Video_GIF(ui->spinBox_ThreadNum_gif_internal->value());
    QProcess *Waifu2x = new QProcess();
    QString program = resolveEnginePath(Current_Path, "realesrgan-ncnn-vulkan", "realesrgan-ncnn-vulkan");
    bool waifu2x_qprocess_failed = false;
    int DenoiseLevel_tmp = DenoiseLevel;
...
int MainWindow::RealESRGAN_NCNN_Vulkan_Video(int rowNum)
{
    ...
    {
        emit Send_video_write_VideoConfiguration(VideoConfiguration_fullPath,ScaleRatio,DenoiseLevel,CustRes_isEnabled,CustRes_height,CustRes_width,"realesrgan-ncnn-vulkan",false,"","",ui->groupBox_FrameInterpolation->isChecked(),0);
    }
    //=======================
    //   检测缓存是否存在
    //=======================
    ...
    //读取配置讯息
    QString RealESRGAN_NCNN_Vulkan_Settings_str = RealESRGAN_NCNN_Vulkan_ReadSettings_Video_GIF(ui->spinBox_ThreadNum_video_internal->value());
    QProcess *Waifu2x = new QProcess();
    QString program = resolveEnginePath(Current_Path, "realesrgan-ncnn-vulkan", "realesrgan-ncnn-vulkan");
    bool waifu2x_qprocess_failed = false;
    int DenoiseLevel_tmp = DenoiseLevel;
...
int MainWindow::RealESRGAN_NCNN_Vulkan_Video_BySegment(int rowNum)
{
    ...
    {
        emit Send_video_write_VideoConfiguration(VideoConfiguration_fullPath,ScaleRatio,DenoiseLevel,CustRes_isEnabled,CustRes_height,CustRes_width,"realesrgan-ncnn-vulkan",true,VideoClipsFolderPath,VideoClipsFolderName,ui->groupBox_FrameInterpolation->isChecked(),0);
    }
    //=======================
    //   检测缓存是否存在
    //=======================
...
QString MainWindow::RealESRGAN_NCNN_Vulkan_PreLoad_Settings()
{
    ...
    if(ui->checkBox_TTA_RealESRGAN->isChecked()==false)
    {
        //==========单显卡==========
        //GPU ID
        if(ui->comboBox_GPUID_RealESRGAN->currentText()!="auto")
        {
            RealESRGAN_NCNN_Vulkan_Settings_str.append("-g "+ui->comboBox_GPUID_RealESRGAN->currentText()+" ");
        }
        //Tile Size
        RealESRGAN_NCNN_Vulkan_Settings_str.append("-t "+QString::number(ui->spinBox_TileSize_RealESRGAN->value(),10)+" ");
    }
    //TTA
    ...
    //Model - depends on image style, select from 2D or 3D combobox
    //Model is NOT cached in PreLoad because it depends on per-image style
    RealESRGAN_NCNN_Vulkan_Settings_str.append("-j 1:1:1 ");
    //=======================================
...
读取配置生成配置QString
*/
QString MainWindow::RealESRGAN_NCNN_Vulkan_ReadSettings()
{
    QString RealESRGAN_NCNN_Vulkan_Settings_str = "";
    ...
    if(ui->checkBox_TTA_RealESRGAN->isChecked())
    {
        ...
        //GPU ID
        RealESRGAN_NCNN_Vulkan_Settings_str.append("-g "+GPUInfo["ID"]+" ");
        //Tile Size
        RealESRGAN_NCNN_Vulkan_Settings_str.append("-t "+GPUInfo["TileSize"]+" ");
    }
...
int MainWindow::RealESRGAN_ncnn_vulkan_DetectGPU()
{
    ...
    QString engine_folder = resolveEngineDirectory(Current_Path, "realesrgan-ncnn-vulkan");
    QString program = resolveEnginePath(Current_Path, "realesrgan-ncnn-vulkan", "realesrgan-ncnn-vulkan");
    QString model_path = engine_folder+"/models/realesrgan-x4plus";
    //===========
    int GPU_ID=-1;
    //=========
    while(true)
...
int MainWindow::RealESRGAN_ncnn_vulkan_DetectGPU_finished()
{
    ...
    if(!Available_GPUID_RealESRGAN_ncnn_vulkan.isEmpty())
    {
        ...
        {
            ui->comboBox_GPUID_RealESRGAN->addItem(Available_GPUID_RealESRGAN_ncnn_vulkan.at(i));
            AddGPU_MultiGPU_RealESRGAN(Available_GPUID_RealESRGAN_ncnn_vulkan.at(i));
        }
    }
    //====
...
/*
RealESRGAN_NCNN_Vulkan_MultiGPU
...
向多显卡的显卡信息列表中添加显卡信息
*/
void MainWindow::AddGPU_MultiGPU_RealESRGAN(QString GPUID)
{
    QMap<QString,QString> GPUInfo;
    GPUInfo["ID"] = GPUID;
...
void MainWindow::on_checkBox_isEnable_CurrentGPU_MultiGPU_RealESRGAN_clicked()
{
    ...
    {
        QMap<QString,QString> GPUInfo_tmp = GPUIDs_List_MultiGPU_RealESRGAN.at(i);
...
{
    int Max_GPU_ID = GPUIDs_List_MultiGPU_RealESRGAN.size();
    QString MultiGPUSettings_str="";
    for(int GPU_ID=0; GPU_ID<Max_GPU_ID; GPU_ID++)
    {
        QMap<QString,QString> GPUInfo = GPUIDs_List_MultiGPU_RealESRGAN.at(GPU_ID);
        if(GPUInfo["isEnabled"] != "true")
        {
            continue;
        }
...
RealESRGAN 块大小调整按钮
*/
void MainWindow::on_pushButton_Add_TileSize_RealESRGAN_clicked()
{
    ui->spinBox_TileSize_RealESRGAN->setValue(AddTileSize_NCNNVulkan_Converter(ui->spinBox_TileSize_RealESRGAN->value()));
}

void MainWindow::on_pushButton_Minus_TileSize_RealESRGAN_clicked()
{
...
*/
QMap<QString,int> MainWindow::Calculate_ScaleRatio_RealESRGAN_NCNNVulkan(int ScaleRatio)
{
...
QString MainWindow::RealESRGAN_NCNN_Vulkan_ReadSettings_Video_GIF(int ThreadNum)
{
    ...
    if(ui->checkBox_TTA_RealESRGAN->isChecked()==false)
    {
        ...
        RealESRGAN_NCNN_Vulkan_Settings_str.append("-j "+jobs_num_str+":"+jobs_num_str+":"+jobs_num_str+" ");
    }
    else
    {
        //==============多显卡================
        int NumOfGPUs_Available = GPUIDs_List_MultiGPU_RealESRGAN.size();//获取显卡总数
        int NumOfGPUs_Enabled = 0;
        QString GPU_ID_cmd = "-g ";
        QString TileSize_cmd = "-t ";
        for(int i=0; i<NumOfGPUs_Available; i++)
        {
            QMap<QString,QString> GPUInfo = GPUIDs_List_MultiGPU_RealESRGAN.at(i);
        ...
        RealESRGAN_NCNN_Vulkan_Settings_str.append(GPU_ID_cmd+" ");
        RealESRGAN_NCNN_Vulkan_Settings_str.append(TileSize_cmd+" ");
        //线程数量
        ...
        RealESRGAN_NCNN_Vulkan_Settings_str.append(QString("-j %1:").arg(LoadAndWrite_tnum)+Jobs_cmd+QString(":%1 ").arg(LoadAndWrite_tnum));
    }
    //TTA
    if(ui->checkBox_TTA_RealESRGAN->isChecked())
    {
...
放大 APNG
*/
bool MainWindow::APNG_RealESRGAN_NCNNVulkan(QString splitFramesFolder,QString scaledFramesFolder,QString sourceFileFullPath,QStringList framesFileName_qStrList,QString resultFileFullPath)
{
    //生成文件夹
